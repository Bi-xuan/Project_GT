...
find_best_split <- function(X, y) {
  ...
  for (threshold in unique_values[-length(unique_values)]) {
    ...
    if (sum(left_indices) > 0 && sum(right_indices) > 0) {
      ...
      impurity_reduction <- current_impurity - (sum(left_indices) / m) * left_impurity -
        (sum(right_indices) / m) * right_impurity
...
new_obs <- c(1.2, -4, 3, -4.41443507454012, -3.98353225341726)
